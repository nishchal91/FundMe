...
<?php
    ...
    $sql_recommended_projects = "SELECT * FROM PROJECT
              JOIN PROJECT_CATEGORY ON PROJECT.PROJECT_ID = PROJECT_CATEGORY.PROJECT_ID
            WHERE PROJECT_CATEGORY.CATEGORY_ID IN (
              SELECT PROJECT_CATEGORY.CATEGORY_ID FROM PROJECT_LIKES
                JOIN PROJECT ON PROJECT.PROJECT_ID = PROJECT_LIKES.PROJECT_ID
                JOIN PROJECT_CATEGORY ON PROJECT.PROJECT_ID = PROJECT_CATEGORY.PROJECT_ID
              WHERE UPPER(EMAIL_ID) = UPPER('$email')
            );";


    $recommended_projects_result = $conn->query($sql_recommended_projects);

    if ($recommended_projects_result->num_rows === 0) {?>
        <script>
            alert("No recommended projects!");
            window.location.href = 'homepage.php';
        </script>
    <?php
    } else {
        while ($row = $recommended_projects_result->fetch_assoc()) { ?>
        <center>
            <div class ="col-sm-4">
                <div class="card">
