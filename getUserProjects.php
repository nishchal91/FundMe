...
$user_projects_query = "SELECT PROJECT.PROJECT_ID,PROJECT.PROJECT_TITLE,PROJECT.DESCRIPTION,PROJECT.PHOTO
from PROJECT
  join PROJECT_OWNERSHIP on PROJECT.PROJECT_ID = PROJECT_OWNERSHIP.PROJECT_ID
where PROJECT_OWNERSHIP.EMAIL_ID = '$email'";


$user_projects_result = $conn->query($user_projects_query);

if ($user_projects_result->num_rows === 0) {
    ?>
    <script>
        alert("You have not posted any projects yet.");
